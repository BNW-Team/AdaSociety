...
def get_contract_env_config(args):
     resource_list = [
          {
               Resource.NAME: "wood",
     ...
     event_list = [
          {Event.NAME: "hammercraft"} for i in range(41)
     ]
     env_config = {
          ...
          'record': args.record,
          'record_dir': f'./results/record/Contract_{args.algo}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv',
     }
