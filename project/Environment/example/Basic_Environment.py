...
class ID2Action:
    ...
    def __init__(self, 
        ...
        self.link_player = (6 + 2*Resource_num + communication_dim, 
        ...
        for action_id in range(self.communication[1] - self.communication[0]):
            ...
            self.action_list.append(
                self.communication_action([players[0], players[1]], tempzero)
                )
            
        for action_id in range(self.link_player[1] - self.link_player[0]):
            self.action_list.append(
    ...
    def communication_action(self, player, content):
        def _communicate(*args):
            return Communication_Action([player[0], player[1]], content)
        return _communicate
...
class BraveNewWorld(MultiAgentEnv):
    ...
    def __init__(self, config):
        ...
        self.observation_space = {
            player.name: Dict({
                ...
                'action_mask': Box(0,1, (2*self.Resources_num + 6 + self.communication_words_dim,))
            }) for player in self.chunk.Player
        }
        self.action_space = {player.name: Discrete(2*self.Resources_num + 6 + self.communication_words_dim)
                             for player in self.chunk.Player}
